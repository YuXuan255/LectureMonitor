...
- `main_user`：登录失效提醒邮件收件人（与常规通知收件人分离）
...
- 若当前是无头模式并检测到登录失效，脚本会提示你按回车，然后临时切换到有头模式弹出窗口让你登录；本轮结束后自动关闭该窗口并回到无头模式。
- 登录后如果页面被重定向到大厅首页，请直接根据终端提示在终端中按下回车，脚本会自动多次跳回目标检索页并重试检测。

## 邮件通知规则

- 仅在检测到“新活动”时发送邮件。
- 登录失效提醒只发送给 `main_user` 配置中的邮箱列表，避免打扰其他收件人。
